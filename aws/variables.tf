...
variable "availability_zones" {
  type = map(object({
    ...
    private_rds_az = optional(bool, true)
  }))
  description = <<-EOF
  ...
  validation {
    condition = length([for az, cfg in var.availability_zones : az if cfg.private_rds_az]) >= 2
    error_message = <<-EOF
    RDS requires at least 2 availability zones to deploy an RDS instance.
    Consider adding at least 2 private AZs with the flag `private_rds_az` set to `true`.
    EOF
  }
