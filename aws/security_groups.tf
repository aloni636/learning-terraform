...
resource "aws_security_group" "my_ip_inbound_ssh" {
  name        = "my_ip_inbound_ssh"
  description = "Allow inbound SSH only from my IP and any outbound traffic"
  vpc_id      = aws_vpc.vpc.id

  tags = {
    Name = "allow-my-ip-inbound-ssh"
  }
}

resource "aws_security_group" "all_outbound_ipv4" {
  name        = "all_outbound_ipv4"
...
resource "aws_vpc_security_group_ingress_rule" "my_ip_inbound_ssh" {
  security_group_id = aws_security_group.my_ip_inbound_ssh.id
  ...
  tags = {
    Name = "allow-my-ip-inbound-ssh-rule"
  }
}
...
resource "aws_vpc_security_group_egress_rule" "all_outbound_ipv4" {
  security_group_id = aws_security_group.all_outbound_ipv4.id
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1" # semantically equivalent to all protocols

  tags = {
    Name = "allow-all-outbound-ipv4-rule"
  }
}


# ----- Private SSM SGs ----- #
resource "aws_security_group" "outbound_ssm" {
  name        = "outbound_ssm"
  description = "Allow HTTPS from private instances to SSM endpoints"
  vpc_id      = aws_vpc.vpc.id

  tags = {
    Name = "allow-outbound-ssm"
  }
}

# SSM requires port 443; See: https://docs.aws.amazon.com/general/latest/gr/ssm.html#ssm_region
resource "aws_vpc_security_group_egress_rule" "outbound_ssm" {
  security_group_id = aws_security_group.outbound_ssm.id
  from_port         = 443
  to_port           = 443
  ip_protocol       = "tcp"
  ...
  referenced_security_group_id = aws_security_group.inbound_ssm.id
}

resource "aws_security_group" "inbound_ssm" {
  name        = "inbound_ssm"
  description = "Allow HTTPS from private instances to SSM endpoints"
  vpc_id      = aws_vpc.vpc.id

  tags = {
    Name = "allow-inbound-ssm"
  }
}

resource "aws_vpc_security_group_ingress_rule" "inbound_ssm" {
  security_group_id = aws_security_group.inbound_ssm.id
  from_port         = 443
  to_port           = 443
  ip_protocol       = "tcp"
  /* Different from classical cidr block notation, this security rule means that 
     only resources with outbound SSM security group can access SSM endpoints */
  referenced_security_group_id = aws_security_group.outbound_ssm.id

  tags = {
    Name = "allow-inbound-ssm-rule"
  }
}


# ----- Private S3 Access SGs ----- #
resource "aws_security_group" "outbound_s3" {
  name        = "outbound_s3"
  description = "Allow HTTPS from private instances to S3 gateway endpoint"
  vpc_id      = aws_vpc.vpc.id

  tags = {
    Name = "allow-outbound-s3"
  }
}
...
resource "aws_vpc_security_group_egress_rule" "outbound_s3_gateway" {
  ...
  security_group_id = aws_security_group.outbound_s3.id
  ...
  tags = {
    Name = "allow-outbound-s3-gateway-rule"
  }
}
...
resource "aws_security_group" "outbound_rds" {
  ...
  tags = {
    Name = "allow-outbound-rds"
  }
}

resource "aws_vpc_security_group_egress_rule" "outbound_rds" {
